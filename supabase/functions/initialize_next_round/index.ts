...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const nonCzarMembers = members.filter((m) => m.user_id !== room.metadata.czar_id);
    ...
    const { data: allHandCards, error: handErr } = await supabase
      ...
      .eq('room_id', room_id)
      .in('user_id', nonCzarMembers.map((m) => m.user_id));
    ...
    const cardNeeds = nonCzarMembers.map((m) => ({
      user_id: m.user_id,
    ...
    if (totalNeeded > 0) {
      ...
      if (pool.length > 0) {
        ...
        let cursor = 0;
        const handInserts: { user_id: string; card_id: string, room_id: string }[] = [];
