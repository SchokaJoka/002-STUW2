...
<template>
    <div class="relative w-full flex flex-row items-center justify-between gap-4 bg-neutral-50 p-5 rounded-lg border border-[3px] border-black">
        <transition name="edit-fade" mode="out-in" @after-enter="handleAfterEnter">
            ...
            <input v-else ref="editInput" v-model="editingText" @keyup.enter="($event.target as HTMLInputElement | null)?.blur()" @blur="save()" type="text"
            class="w-full text-black px-4 py-2 text-xl font-semibold bg-transparent border-none focus:ring-0" />
        </transition>
            <div class="flex flex-row gap-2 items-center">
                <div class="hover:cursor-pointer" :class="!canEdit && !isEditing ? 'opacity-40 pointer-events-none' : ''" @click="startEdit()">
